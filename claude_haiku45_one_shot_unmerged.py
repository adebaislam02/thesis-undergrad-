# -*- coding: utf-8 -*-
"""
Claude Haiku 4.5 — one-shot UNMERGED example, full run.

Byte-identical one-shot prompt template to one_shot_gpt_unmerged.py
(GPT-4o-mini), oneshot_unmerged_deepseek.py, and oneshot_unmerged_mistral.py.
Only intentional deltas from those scripts:
...
The in-context example is dataset row index 215 (mui/material-ui/pull/37163) —
the same unmerged example used across all three previous models. That row
is excluded from the eval set (matches prev models), yielding 299 output rows.
"""
...
OUTPUT_FILE = "_HAIKU45_batch_one_shot_unmerged_300.csv"

EXAMPLE_INDEX = 215  # mui/material-ui/pull/37163 — same unmerged example as GPT/DeepSeek/Mistral
...
def batch_process_one_shot(df, example_index, output_path, api_key):
    ...
    for idx, row in tqdm(df.iterrows(), total=len(df), desc="Haiku-4.5 one-shot unmerged"):
    ...
    print(f"\nOne-shot unmerged run complete. Results saved to: {output_path}")
