...
# 1) Read your CSV (or JSONL)
df = pd.read_csv(file_path)  # or pd.read_json("annotated_data.jsonl", lines=True)
...
def get_notes_for_example(row: pd.Series) -> str:
    """
    Return notes for example PR prompt:
    Prefer 'Notes (Annotator 2)' if non-empty,
    otherwise fall back on 'Notes (Annotator 1)',
    else empty string.
    """
...
def call_gpt_4o_mini(prompt):
    ...
    if response.status_code == 200:
        ...
        # Print the entire API response to debug
        # print("API Response:")
        # print(json.dumps(response_json, indent=4))  # Pretty-print the response for clarity
        ...
        if "choices" in response_json and len(response_json["choices"]) > 0:
            ...
            if content:
                try:
                    ...
                    print(f"Error parsing the JSON content: {content}")
                    return None
            else:
        ...
        print(response.text)  # Print the error message if the status code is not 200
        return None
...
def render_pr_block(row: pd.Series, use_notes: bool = True, is_example: bool = False) -> str:
    """
    Render PR fields into a prompt text block.

    Parameters:
        row: The PR data row (pandas Series).
        use_notes: Whether to include notes in output.
        is_example: Whether this block is for example PR
                    (True: show notes, False: omit notes).

    Returns:
        A single string representing the PR block.
    """
    def or_none(x):
...
def build_one_shot_prompt(df: pd.DataFrame, example_index: int, target_row: pd.Series) -> str:
    example = df.iloc[example_index]
    ex_block = render_pr_block(example, use_notes=True)
...
def batch_process_one_shot(df: pd.DataFrame, example_index: int = 0, output_path: str = 'batch_one_shot_results.csv') -> pd.DataFrame:
    ...
    for idx, row in tqdm(df.iterrows(), total=len(df), desc="One-Shot Batch"):
        if idx == example_index or idx in processed_indices:
            continue  # Skip example and already processed PRs

        prompt = build_one_shot_prompt(df, example_index, row)

        try:
            raw = call_gpt_4o_mini(prompt)
            if raw:
                result = {
                    "index": idx,
...
#     output_path = 'batch_one_shot_results_for_300_gpt4o_unmerged.csv'
#     results_df = batch_process_one_shot(df, example_index, output_path=output_path)
#     print(f"Results saved to {output_path}")
#     print(results_df.head())

def rerun_failed_indices(df, failed_indices, example_index, output_path, api_key):
    records = []
    for idx in failed_indices:
        ...
        try:
            ...
            if raw:
                result = {
                    ...
                    "green_flags": raw.get("green_flags", []),
                    "error": None,
                }
            else:
                result = {
        ...
        records.append(result)
        pd.DataFrame(records).to_csv(output_path, index=False)
        print(f"Re-processed PR index {idx} - results saved to {output_path}")
    return pd.DataFrame(records)
...
if __name__ == "__main__":
    ...
    # results_df = batch_process_one_shot(df, example_index, output_path='batch_one_shot_results.csv')

    # Then re-run only failed rows:
    failed_indices = [218]
    rerun_results = rerun_failed_indices(df, failed_indices, example_index, "GPT_RERUN_failed_one_shot.csv", API_KEY)

    print("Re-run results saved to rerun_failed_one_shot2.csv")
