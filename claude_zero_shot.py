...
_surrogate_regex = re.compile(r'[\ud800-\udfff]')

def remove_surrogates(text):
    if not isinstance(text, str):
        return text
    # Remove any surrogate half code points
    return _surrogate_regex.sub('', text)
...
def load_and_preprocess_data(filepath):
    ...
    for col in text_cols:
     if col in df.columns:
        df[col] = df[col].apply(clean_text_field).apply(remove_surrogates)
...
def sanitize_record(record):
    for key, val in record.items():
        if isinstance(val, str):
            record[key] = sanitize_text(val)
        if isinstance(val, list):
            # Sanitize each string element in lists (e.g. red_flags, green_flags)
            record[key] = [sanitize_text(v) if isinstance(v, str) else v for v in val]
    return record
...
def batch_process_zero_shot(df: pd.DataFrame, output_path: str = 'batch_zero_shot_results_gemini.csv') -> pd.DataFrame:
    ...
    for idx, row in tqdm(df.iterrows(), total=len(df), desc="Zero-Shot Batch"):
        ...
        try:
            raw = call_claude_3_7(prompt)
            if raw:
                parsed = extract_json(raw)
                result = {
                    "index": idx,
                    "accepted": sanitize_text(parsed.get("accepted")),
                    "red_flags": [sanitize_text(flag) for flag in parsed.get("red_flags", [])],
                    "green_flags": [sanitize_text(flag) for flag in parsed.get("green_flags", [])],
                    "error": None,
                }
            else:
                result = {
...
#     output_path = '_CLAUDE_batch_zero_shot_300.csv'
#     results_df = batch_process_zero_shot(df, output_path=output_path)
#     print(f"Results saved to {output_path}")
#     print(results_df.head())


def rerun_failed_prs_by_index(
    df, failed_indices, output_file, api_key, prompt_builder, call_model_func
):
    import pandas as pd
    ...
    for idx in failed_indices:
        ...
        try:
            ...
            if raw:
                # Sanitize to avoid UTF-8 surrogate errors
        ...
        # Save after each rerun to avoid data loss
...
def main():
    ...
    # Provide your failed PR indices here
    failed_indices = [18]

    output_file = "_CLAUDE_failed_prs_rerun_results.csv"

    rerun_df = rerun_failed_prs_by_index(
        df=df,
        failed_indices=failed_indices,
        output_file=output_file,
        api_key=API_KEY,
        prompt_builder=build_zero_shot_prompt,  # or other prompt builder function
        call_model_func=call_claude_3_7           # or your model call function
    )

    print("\nSample rerun results:")
    print(rerun_df.head())
