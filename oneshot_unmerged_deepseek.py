...
API_KEY = "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"  # Replace with your OpenRouter API key
...
def call_deepseek_with_retries(prompt, max_retries=5, base_delay=1):
    headers = {
    ...
    for attempt in range(max_retries):
        ...
        if response.status_code == 200:
            ...
            # Rate limited, apply exponential backoff retry
...
def batch_process_one_shot(df: pd.DataFrame, example_index: int = 0, output_path: str = 'batch_one_shot_results.csv') -> pd.DataFrame:
    ...
    for idx, row in tqdm(df.iterrows(), total=len(df), desc="One-Shot Batch"):
        ...
        try:
            raw = call_deepseek_with_retries(prompt)
            if raw:
                result = {
                    "index": idx,
...
if __name__ == "__main__":
    ...
    failed_indices = [117]
    rerun_results = rerun_failed_indices(df, failed_indices, example_index, "DEpseek_fled_one_shot.csv", API_KEY)
