# # -*- coding: utf-8 -*-
# """oneShot_using_merged_example.ipynb

# Automatically generated by Colab.

# Original file is located at
#     https://colab.research.google.com/drive/1G8X1UzvagSrF_xJRMfuTb65jdXRDp2DP
# """
# import requests
# import pandas as pd
# import json
...
# failed_indices = [139,140,141,142,143,144,145,146,147]
...
# # Usage example:
# rerun_failed_indices(df, failed_indices, example_index=7, output_path="rerun_failed_one_shot.csv", api_key=API_KEY)
# if __name__ == "__main__":
#     df = load_and_preprocess_data(file_path)
#     example_index = 7

#     # Run full batch once (optional)
#     # results_df = batch_process_one_shot(df, example_index, output_path='batch_one_shot_results.csv')

#     # Then re-run only failed rows (replace with your actual failed indices)
#     failed_indices = [139,140,141,142,143,144,145,146,147]
#     rerun_results = rerun_failed_indices(df, failed_indices, example_index, "rerun_failed_one_shot.csv", API_KEY)
    
#     print("Re-run results saved to rerun_failed_one_shot.csv")
...
# #     output_path = 'batch_one_shot_results_merged_deepseek.csv'
...
def load_and_preprocess_data(filepath):
    ...
    # Parsing operations for list-type columns
    for col in ["pr_commit_messages", "pr_comments", "pr_review_comments"]:
        if col in df.columns:
            df[col] = df[col].apply(lambda x: parse_list_and_join(x, sep="\n\n"))

    # Clean annotator 1 flags columns
    for flag_col in ["Green Flags (Annotator 1)", "Red Flags (Annotator 1)"]:
        if flag_col in df.columns:
            df[flag_col] = df[flag_col].apply(parse_and_join_flags)

    # Create unified final-or-fallback columns with 'FF' suffix
    if "Green Flags (Final)" in df.columns and "Green Flags (Annotator 1)" in df.columns:
        df["Green Flags FF"] = df.apply(
            lambda row: parse_and_join_flags(get_final_or_fallback(row, "Green Flags (Final)", "Green Flags (Annotator 1)")),
            axis=1
