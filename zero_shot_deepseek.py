        ...
        if "choices" in response_json and len(response_json["choices"]) > 0:
            ...
            if content:
                try:
                    ...
                    print("Error parsing model output:", content)
                    return None
            else:
                print("Empty content field in API response.")
    else:
...
def build_zero_shot_prompt(row: pd.Series) -> str:
    """
    Build zero-shot prompt with same structure as one-shot but without example.
    """
...
def batch_process_zero_shot(df: pd.DataFrame, output_path: str = 'batch_zero_shot_results_deepseek.csv') -> pd.DataFrame:
...
if __name__ == "__main__":
    ...
    output_path = 'batch_zero_shot_results_deepseek.csv'
...
# if __name__ == "__main__":
#     df = load_and_preprocess_data(file_path)
#     target_indices = [41, 44, 57]  # choose the PR indices you want

#     output_path = 'selected_pr_results.csv'
#     results = []

#     for idx in target_indices:
#         row = df.iloc[idx]
#         prompt = build_zero_shot_prompt(row)  # or build_one_shot_prompt(...), as needed

#         try:
#             raw = call_deepseek(prompt)
#             result = {
#                 "index": idx,
#                 "accepted": raw.get("accepted") if raw else None,
#                 "red_flags": raw.get("red_flags") if raw else [],
#                 "green_flags": raw.get("green_flags") if raw else [],
#                 "error": None if raw else "API Call failed"
#             }
#         except Exception as e:
#             result = {
#                 "index": idx,
...
#         print(f"Processed PR index {idx}")

#     pd.DataFrame(results).to_csv(output_path, index=False)
#     print(f"Selected PR results saved to {output_path}")
