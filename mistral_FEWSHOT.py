...
MODEL_NAME = "mistralai/mistral-small-3.2-24b-instruct"  # Replace OpenAI model with Mistral
INPUT_FILE = "FINAL_DATASET - 300_Annotated.csv"
OUTPUT_FILE = "_MISTRAL_batch_fewSHOT.csv"
EXAMPLE_INDICES = [88, 140, 261]

OUTPUT_FILE = "_MISTRAL_batch_fewSHOT_rerun_failed.csv"
FAILED_PR_INDICES = [84, 175, 246]  # List of failed PR indices
...
def process_via_mistral(prompt, api_key, model=MODEL_NAME, max_tokens=512):
...
def batch_process_few_shot(
    ...
    for idx, row in tqdm(df.iterrows(), total=len(df), desc="Few-Shot Batch"):
        ...
        try:
            raw = process_via_mistral(prompt, api_key)
            if raw:
...
def rerun_failed_prs(df: pd.DataFrame, failed_indices: list, output_path: str = "rerun_failed_prs.csv", api_key: str = None):
    records = []
    for idx in tqdm(failed_indices, desc="Rerunning Failed PRs"):
        row = df.iloc[idx]
        prompt = build_few_shot_prompt(df, EXAMPLE_INDICES, row)
    ...
    # Save the rerun results to the output CSV
...
if __name__ == "__main__":
    df = load_and_preprocess_data(INPUT_FILE)
    rerun_failed_prs(df, FAILED_PR_INDICES, OUTPUT_FILE, OPENROUTER_API_KEY)
