# -*- coding: utf-8 -*-
"""
DeepSeek R1 — zero-shot RE-RUN.

Rebuilds `batch_zero_shot_results_deepseek.csv` cleanly. Original file was
CSV-corrupted (mixed quote styles → ~23 unrecoverable rows). This script:
  - Uses the SAME zero-shot prompt as the original zero_shot_deepseek.py
    (byte-identical — matches GPT/Mistral zero-shot too)
  - Uses `deepseek/deepseek-r1` (the `:free` tier the original used is no
    longer available on OpenRouter — this is the current stable R1 model)
  - Applies Haiku-style safeguards: inline retry + Unicode surrogate strip
  - Writes clean CSV (same 5-column schema as original: index, accepted,
    red_flags, green_flags, error) plus token cols for cost visibility
  - Keyed by `index` alone (matches the other old-model CSVs)
"""
...
OUTPUT_FILE = "batch_zero_shot_results_deepseek.csv"
...
def call_deepseek_with_retries(prompt, api_key, model=MODEL_NAME, max_tokens=4096, max_retries=MAX_RETRIES):
    """DeepSeek R1 is a reasoning model — allow larger max_tokens to accommodate CoT."""
...
def batch_process(df, output_path, api_key):
    if not api_key:
        raise SystemExit("OPENROUTER_API_KEY not set — put it in .env and re-run.")

    if os.path.exists(output_path):
        prev = pd.read_csv(output_path, engine="python", on_bad_lines="skip")
        processed = set(pd.to_numeric(prev["index"], errors="coerce").dropna().astype(int).tolist())
        records = prev.to_dict("records")
        print(f"Resuming: {len(processed)} indexes already processed")
    else:
        processed = set()
        records = []

    for idx, row in tqdm(df.iterrows(), total=len(df), desc="DeepSeek R1 zero-shot"):
        ...
        try:
            raw, usage = call_deepseek_with_retries(prompt, api_key)
            ...
            print(f"[ERROR @ idx={idx}] {result['error']}")

        records.append(result)
        pd.DataFrame(records).drop_duplicates(subset=["index"]).to_csv(output_path, index=False)

    print(f"\nDeepSeek zero-shot rerun complete. Results saved to: {output_path}")
...
if __name__ == "__main__":
    ...
    # Blow away the corrupted file (backup was already made)
    if os.path.exists(OUTPUT_FILE):
        os.remove(OUTPUT_FILE)
        print(f"Removed old corrupted {OUTPUT_FILE} (backup at {OUTPUT_FILE}.corrupted_backup)")
    results = batch_process(df, OUTPUT_FILE, API_KEY)
    print("\nSample results:")
    print(results[["index", "accepted", "error"]].head())
